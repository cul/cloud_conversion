...
set :application, 'cloud_conversion'
set :repo_name, fetch(:application)
# set :repo_url, 'git@github.com:cul/cloud_conversion.git'
set :repo_url, 'https://github.com/cul/cloud_conversion.git' # use https for now, until we have ssh keys set up properly
...
append  :linked_dirs,
        ...
        'tmp/pids',
        'node_modules',
        # Adding storage so that local sqlite databases persist between deployments
        'storage'
...
after 'deploy:finished', 'cloud_conversion:restart_resque_workers'
