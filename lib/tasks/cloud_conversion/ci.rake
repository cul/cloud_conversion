namespace :cloud_conversion do
  ...
  RSpec::Core::RakeTask.new(:rspec) do |spec|
    # We don't have any test types that we want to exclude right now,
    # so 'placeholder' is...just a placeholder!
    exclusions = ENV['EXCLUDE_SPECS'].to_s.split(',') & ['placeholder']
  ...
  task ci_nocop: ['cloud_conversion:setup:config_files', :environment, 'cloud_conversion:ci_specs']

  desc 'CI build with Rubocop validation'
  task ci: ['cloud_conversion:setup:config_files', :environment, 'cloud_conversion:rubocop', 'cloud_conversion:ci_specs']
  ...
  task ci_specs: :environment do
    ...
    duration = Benchmark.realtime do
      ...
      begin
        Rake::Task['cloud_conversion:rspec'].invoke
      rescue SystemExit => e
